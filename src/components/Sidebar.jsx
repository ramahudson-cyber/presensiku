...
export default function Sidebar({ menuOpen = false, setMenuOpen = () => {} }) {
  ...
  const adminMenus = [
    ...
    { path: "/admin/schedules", label: "Jadwal", icon: CalendarDays },
  ...
  return (
    <>
      ...
      <aside
        className={`fixed top-0 left-0 h-full w-[270px] bg-[#4A2E7A] text-white flex flex-col z-40 transition-transform duration-300
      ...
      >
        ...
        <div className="p-5 flex items-center justify-between border-b border-white/10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold">SIAP</h1>
              <p className="text-[10px] text-violet-300">Puskesmas Ampenan</p>
            </div>
          </div>
          <button onClick={() => setMenuOpen(false)} className="md:hidden p-1 text-violet-300 hover:text-white">
            <X size={20} />
          </button>
        </div>
        ...
        <div className="px-5 py-3 border-b border-white/10">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-full bg-gradient-to-br from-violet-400 to-purple-500 flex items-center justify-center text-xs font-bold shrink-0">
              {user?.full_name?.charAt(0) || "U"}
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-xs font-semibold truncate">{user?.full_name || "User"}</p>
              <p className="text-[9px] text-violet-300 capitalize">{userRole.replace("_", " ")}</p>
            </div>
          </div>
        </div>
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  `flex items-center gap-3 px-3 py-2.5 rounded-xl transition text-sm ${
                    isActive
                      ? "bg-[#6B4BA3] text-white font-semibold shadow-lg"
        ...
        <div className="p-3 border-t border-white/10">
          <button
            ...
            className="flex items-center gap-3 w-full px-3 py-2.5 text-red-300 hover:bg-red-500/10 rounded-xl transition text-sm"
          >
            <LogOut size={18} />
            <span>Logout</span>
          </button>
