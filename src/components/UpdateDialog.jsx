...
export default function UpdateDialog() {
  ...
  if (!isNative) {
    return (
      ...
      >
        <div className="bg-white border border-slate-200 rounded-3xl w-full max-w-sm shadow-2xl animate-fade-in">
          <div className="p-5 border-b border-slate-200 flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-bold text-slate-800">Update Tersedia</h3>
          ...
          <div className="p-5 space-y-4">
            {update.changelog && (
              <div>
                ...
                <p className="text-xs text-slate-600 leading-relaxed">{update.changelog}</p>
              </div>
            )}
            ...
            <button onClick={handleRefresh}
              className="w-full py-3 rounded-full text-sm font-semibold flex items-center justify-center gap-2 transition-all bg-gradient-to-r from-electric-violet to-purple-600 text-white hover:shadow-lg active:scale-[0.98]">
            ...
            <div className="bg-red-50 border border-red-200 rounded-2xl p-3 space-y-2">
              ...
              {permissionRequired && (
                <button onClick={openInstallSettings}
                  className="w-full py-2 rounded-full text-[11px] font-semibold flex items-center justify-center gap-1.5 bg-red-100 text-red-700 border border-red-200 hover:bg-red-200 active:scale-[0.98]">
                  <Settings size={13} /> Buka Pengaturan
                </button>
              )}
              {done && !installerOpened && (
                <button onClick={handleManualInstall}
                  className="w-full py-2 rounded-full text-[11px] font-semibold flex items-center justify-center gap-1.5 bg-electric-violet text-white hover:brightness-110 active:scale-[0.98]">
          ...
          <div className="space-y-2">
            {(downloading || done) && (
              <div className="space-y-2">
                ...
                <p className="text-[10px] text-slate-500 text-center">{progress}%</p>
                {done && installerOpened && (
                  <p className="text-[11px] text-emerald-600 text-center animate-pulse">
